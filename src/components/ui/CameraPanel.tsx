...
export function CameraPanel({ lang }: { lang: Lang }) {
  ...
  return (
    <aside className="ui-layer camera-panel">
      <div className="info-panel__head">
        <h2>{UI.cameraTitle[lang]}</h2>
        <button
          className="icon-btn"
          aria-label={UI.close[lang]}
          onClick={() => toggle("cameraOpen", false)}
        >
          <X size={16} />
        </button>
      </div>
      <div className="explore-grid">
        {keys.map((key) => (
          <button
            ...
            onClick={() => {
              if (key === "sunset" || key === "night") setTime(key);
              focus(CAMERA_PRESETS[key]);
              toggle("cameraOpen", false);
            }}
          >
            {UI.cameraViews[key][lang]}
          </button>
        ))}
      </div>
    </aside>
