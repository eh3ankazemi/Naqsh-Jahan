...
export function Scene({
  ...
  return (
    <Canvas
      shadows={!perfMode}
      dpr={perfMode ? 1 : [1, 1.6]}
      gl={{ antialias: !perfMode, preserveDrawingBuffer: true }}
      camera={{ position: INTRO_VIEW.position, fov: 43, near: 0.5, far: 1100 }}
      onCreated={({ gl }) => {
        gl.toneMapping = THREE.ACESFilmicToneMapping;
        gl.toneMappingExposure = night ? 1.35 : 1.12;
    ...
    >
      <Suspense fallback={null}>
        ...
        <group position={[-110, 0, 0]} rotation-y={Math.PI / 2}>
          <AliQapu glow={TIME_PRESETS[timeOfDay].facadeGlow} />
        </group>
        <group position={[0, 0, -176]}>
          <ImamMosque night={night} />
        </group>
        <group position={[108, 0, 0]} rotation-y={-Math.PI / 2}>
          <SheikhLotfollah night={night} />
        </group>
        <group position={[0, 0, 176]} rotation-y={Math.PI}>
          <QeysariehBazaar perfMode={perfMode} night={night} />
        </group>
